...
async def fetch_article_details(article_url, article_title, page: Page):
    for attempt in range(MAX_RETRIES):
        try:
            ...
            combined_answer = '\n\n'.join(filter(None, [article_text, version_1, version_2, summary]))

            article_id = str(uuid.uuid5(NAMESPACE, article_url))

            return {
                "id": article_id,
                "Question": title or article_title,
...
def constitution_articles():
    ...
    and returns flattened article data in the format: id, Question, Answer, url, category.
    """
    async def _scrape():
        all_articles = []
        ...
        async with async_playwright() as p:
            ...
            for i, part_container in enumerate(part_containers):
                ...
                if not view_all_elem:
                    print(f"[] Skipping {part_name} (no 'View All')")
                    continue
